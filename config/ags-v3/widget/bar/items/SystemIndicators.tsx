...
import {
  config,
  SysIndicatorWidgets,
  qsPopupActive, setQsPopupActive,
} from "../../../lib/config";
...
export default () => {
  ...
  const IdleIndicator = () => {
    if (IdleService) {
      const profile = createBinding(IdleService, "profile");
      return (
        <image
          cssClasses={["system-indicator"]}
  ...
  const NightlightIndicator = () => {
    if (NightLightService) {
      const profile = createBinding(NightLightService, "profile");
      return (
        <image
          cssClasses={["system-indicator"]}
  ...
  const PowerProfileIndicator = () => {
    ...
    return (
      <image
        ...
        iconName={createBinding(power, "iconName")}
      />
    );
  };
          ...
          {(wifi) =>
            wifi && (
              <image
                ...
                visible={createBinding(network, "primary").as(p => p === AstalNetwork.Primary.WIFI)}
              />
            )
          }
        </With>
          ...
          {(wired) =>
            wired && (
              <image
                ...
                visible={createBinding(network, "primary").as(p => p === AstalNetwork.Primary.WIRED)}
              />
            )
          }
        </With>
  ...
  return (
    <button
      cssClasses={qsPopupActive.as(a => a ? ["system-indicators", "bar-button", "active"] : ["system-indicators", "bar-button", "inactive"])}
      onClicked={() => {
        app.toggle_window("quicksettings");
        setQsPopupActive(true);
      }}
      $={self => {
