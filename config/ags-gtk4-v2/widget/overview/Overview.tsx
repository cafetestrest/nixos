import { Astal, Gtk, Gdk, hook } from "astal/gtk4";
...
const Client = ({ address, size: [w, h], class: c, title }) => (
	<button
		...
		setup={(btn) => {
			hook(btn, btn, "drag-data-get", (_w, _c, data) => data.set_text(address, address.length));
			hook(btn, btn, "drag-begin", (_, context) => {
				Gtk.drag_set_icon_surface(context, createSurfaceFromWidget(btn));
				btn.toggleClassName('hidden', true);
			})
			hook(btn, btn, "drag-end", () => btn.toggleClassName('hidden', false))
	...
	>
		<image
			...
			setup={(self) => {
				...
				self.set_from_icon_name(getHyprlandClientIcon(c, icon));
			}}
		/>
	</button>
);
